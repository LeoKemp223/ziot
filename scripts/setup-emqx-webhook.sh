...
report_action_payload=$(
  jq -cn \
    '{
      type: "http",
      name: "ziot_device_report_webhook",
      connector: "ziot_webhook_connector",
      enable: true,
      parameters: {
        method: "post",
...
ota_action_payload=$(
  jq -cn \
    '{
      type: "http",
      name: "ziot_ota_progress_webhook",
      connector: "ziot_webhook_connector",
      enable: true,
      parameters: {
        method: "post",
...
report_rule_payload=$(
  jq -cn \
    '{
      id: "ziot_device_report_events",
      name: "ZiOT device report events",
      enable: true,
      sql: "SELECT topic, payload FROM \"/sys/+/+/thing/property/post\", \"/sys/+/+/thing/event/post\", \"/sys/+/+/thing/log/post\"",
      actions: ["http:ziot_device_report_webhook"]
    }'
)
...
ota_rule_payload=$(
  jq -cn \
    '{
      id: "ziot_ota_progress_events",
      name: "ZiOT OTA progress events",
      enable: true,
      sql: "SELECT topic, payload FROM \"/ota/+/+/upgrade/progress\", \"/ota/+/+/upgrade/result\"",
      actions: ["http:ziot_ota_progress_webhook"]
    }'
)
...
api -X DELETE "$EMQX_API_URL/api/v5/rules/ziot_device_report_events" >/dev/null 2>&1 || true
api -X DELETE "$EMQX_API_URL/api/v5/rules/ziot_ota_progress_events" >/dev/null 2>&1 || true
...
api -X DELETE "$EMQX_API_URL/api/v5/actions/http:ziot_device_report_webhook" >/dev/null 2>&1 || true
api -X DELETE "$EMQX_API_URL/api/v5/actions/http:ziot_ota_progress_webhook" >/dev/null 2>&1 || true
...
api -X POST "$EMQX_API_URL/api/v5/actions" -d "$report_action_payload" >/dev/null
api -X POST "$EMQX_API_URL/api/v5/actions" -d "$ota_action_payload" >/dev/null
...
api -X POST "$EMQX_API_URL/api/v5/rules" -d "$report_rule_payload" >/dev/null
api -X POST "$EMQX_API_URL/api/v5/rules" -d "$ota_rule_payload" >/dev/null
...
echo "EMQX device report webhook configured: $ZIOT_WEBHOOK_BASE_URL/api/internal/emqx/webhook"
echo "EMQX OTA progress webhook configured: $ZIOT_WEBHOOK_BASE_URL/api/internal/emqx/webhook"
